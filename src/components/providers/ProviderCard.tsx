...
export default function ProviderCard({
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 border border-lime-200">
      {provider.image_url && (
        <img src={provider.image_url} alt={provider.name} className="w-full h-40 object-cover rounded mb-2" />
      )}
      ...
      {provider.website && (
        <p className="text-sm text-blue-600 underline">
          <a href={provider.website} target="_blank" rel="noopener noreferrer">Sitio web</a>
        </p>
      )}
      <div className="mt-4 flex gap-2 flex-wrap">
        <button onClick={onEdit} className="text-sm text-white bg-blue-500 px-3 py-1 rounded hover:bg-blue-600">Editar</button>
        <button onClick={onDelete} className="text-sm text-white bg-red-500 px-3 py-1 rounded hover:bg-red-600">Eliminar</button>
        <button onClick={onToggleReviews} className="text-sm text-white bg-green-500 px-3 py-1 rounded hover:bg-green-600">
