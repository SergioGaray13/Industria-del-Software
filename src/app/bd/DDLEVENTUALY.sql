...
CREATE TABLE public.bookings (
  ...
  payment_status text DEFAULT 'pendiente'::text CHECK (payment_status = ANY (ARRAY['pendiente'::text, 'aprobado'::text, 'pagado'::text, 'rechazado'::text])),
  payment_details jsonb,
  hours_reserved integer,
  date_reserved date,
...
CREATE TABLE public.credit_cards (
  ...
  expiration_year integer NOT NULL CHECK (expiration_year::numeric >= EXTRACT(year FROM CURRENT_DATE)),
  cvv text NOT NULL,
...
CREATE TABLE public.salones (
  ...
  precios_por_hora numeric NOT NULL DEFAULT 0,
